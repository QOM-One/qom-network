...
KEY="mykey"
CHAINID="${CHAIN_ID:-qom_7668378-1}"
...
# Change parameter token denominations to aqm
jq '.app_state.staking.params.bond_denom="aqm"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.crisis.constant_fee.denom="aqm"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.gov.deposit_params.min_deposit[0].denom="aqm"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.evm.params.evm_denom="aqm"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.inflation.params.mint_denom="aqm"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
jq -r --arg amount_to_claim "$amount_to_claim" '.app_state.bank.balances += [{"address":"evmos15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"aqm", "amount":$amount_to_claim}]}]' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

# disable produce empty block
sed -i 's/create_empty_blocks = true/create_empty_blocks = false/g' "$CONFIG_TOML"

# Allocate genesis accounts (cosmos formatted addresses)
qomd add-genesis-account $KEY 100000000000000000000000000aqm --keyring-backend $KEYRING
...
qomd gentx $KEY 1000000000000000000000aqm --keyring-backend $KEYRING --chain-id "$CHAINID"
...
qomd start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aqm --json-rpc.api eth,txpool,personal,net,debug,web3
