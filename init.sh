...
# # Set claims decay
# cat $HOME/.qomd/config/genesis.json | jq -r --arg current_date "$current_date" '.app_state["claims"]["params"]["duration_of_decay"]="1000000s"' > $HOME/.qomd/config/tmp_genesis.json && mv $HOME/.qomd/config/tmp_genesis.json $HOME/.qomd/config/genesis.json
...
qomd add-genesis-account $KEY 100000000000000000000010000aqom --keyring-backend $KEYRING
...
qomd gentx $KEY 1000000000000000000000aqom --keyring-backend $KEYRING --chain-id $CHAINID
...
qomd start --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001aqom --json-rpc.api eth,txpool,personal,net,debug,web3 --rpc.laddr "tcp://0.0.0.0:26657" --api.enable