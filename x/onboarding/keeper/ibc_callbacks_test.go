...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	testCases := []struct {
		...
		{
			"no swap / convert all transferred amount - no liquidity pool exists but aqom balance is already bigger than threshold",
			func() {
			...
			sdk.NewCoin("aqom", sdk.NewIntWithDecimal(4, 18)),
			sdk.NewCoin(uusdtIbcdenom, sdk.ZeroInt()),
			transferAmount,
		},
		{
		...
		},
		{
			...
			sdk.NewCoin("aqom", sdk.NewIntWithDecimal(7, 18)),
			sdk.NewCoin(uusdcIbcdenom, sdk.ZeroInt()),
			sdk.NewInt(20998399),
		},
		{
		...
		},
		{
			...
			true,
			sdk.NewCoins(sdk.NewCoin("aqom", sdk.NewIntWithDecimal(3, 18))),
			sdk.NewCoin("aqom", sdk.NewIntWithDecimal(3, 18)),
			sdk.NewCoin(uusdcIbcdenom, sdk.ZeroInt()),
			transferAmount,
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			// Setup liquidity pool (aqom/uUSDC)
			suite.app.CoinswapKeeper.SetStandardDenom(suite.ctx, "aqom")
			coinswapParams := suite.app.CoinswapKeeper.GetParams(suite.ctx)
			coinswapParams.MaxSwapAmount = sdk.NewCoins(sdk.NewCoin(uusdcIbcdenom, sdk.NewIntWithDecimal(10, 6)))
			suite.app.CoinswapKeeper.SetParams(suite.ctx, coinswapParams)

			testutil.FundAccount(suite.app.BankKeeper, suite.ctx, secpAddr, sdk.NewCoins(sdk.NewCoin("aqom", sdk.NewIntWithDecimal(10000, 18)), sdk.NewCoin(uusdcIbcdenom, sdk.NewIntWithDecimal(10000, 6))))
