	...
	Describe("Submitting a token pair proposal through governance", func() {
		Context("with existing coins", func() {
			BeforeEach(func() {
				...
				coins := sdk.NewCoins(
					sdk.NewCoin("aqom", sdk.NewInt(1000000000000000000)),
			...
			Describe("for a single Cosmos Coin", func() {
				BeforeEach(func() {
					...
					_, err = testutil.Delegate(s.ctx, s.app, privKey, sdk.NewCoin("aqom", sdk.NewInt(500000000000000000)), s.validator)
					s.Require().NoError(err)
...
func submitRegisterCoinProposal(ctx sdk.Context, appEvmos *app.Qom, pk *ethsecp256k1.PrivKey, metadata []banktypes.Metadata) (id uint64, err error) {
	content := types.NewRegisterCoinProposal("test Coin", "foo", metadata...)
	return testutil.SubmitProposal(ctx, appEvmos, pk, content, 8)
}

func submitRegisterERC20Proposal(ctx sdk.Context, appEvmos *app.Qom, pk *ethsecp256k1.PrivKey, addrs []string) (id uint64, err error) {
	content := types.NewRegisterERC20Proposal("test token", "foo", addrs...)
	return testutil.SubmitProposal(ctx, appEvmos, pk, content, 8)
}

func convertCoin(ctx sdk.Context, appEvmos *app.Qom, pk *ethsecp256k1.PrivKey, coin sdk.Coin) {
...
func convertERC20(ctx sdk.Context, appEvmos *app.Qom, pk *ethsecp256k1.PrivKey, amt math.Int, contract common.Address) {
	addrBz := pk.PubKey().Address().Bytes()
