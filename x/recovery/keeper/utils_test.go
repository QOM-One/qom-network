...
func (suite *IBCTestingSuite) SetupTest() {
	...
	err = suite.EvmosChain.App.(*app.Qom).BankKeeper.SendCoinsFromModuleToAccount(suite.EvmosChain.GetContext(), inflationtypes.ModuleName, suite.EvmosChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = suite.EvmosChain.App.(*app.Qom).ClaimsKeeper.SetParams(suite.EvmosChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableRecovery = true
	err = suite.EvmosChain.App.(*app.Qom).RecoveryKeeper.SetParams(suite.EvmosChain.GetContext(), params)
	suite.Require().NoError(err)

	evmParams := suite.EvmosChain.App.(*app.Qom).EvmKeeper.GetParams(s.EvmosChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err = suite.EvmosChain.App.(*app.Qom).EvmKeeper.SetParams(s.EvmosChain.GetContext(), evmParams)
	suite.Require().NoError(err)
