...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	coinQom := sdk.NewCoin("aqom", sdk.NewInt(10000))
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			s.EvmosChain.App.(*app.Qom).ClaimsKeeper.SetParams(s.EvmosChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			...
			nativeEvmos := s.EvmosChain.App.(*app.Qom).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, "aqom")
			Expect(nativeEvmos).To(Equal(coinQom))
			ibcOsmo := s.EvmosChain.App.(*app.Qom).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
	...
	Describe("from an authorized, non-EVM chain (e.g. Osmosis)", func() {
		Describe("to a different account on Qom (sender != recipient)", func() {
			BeforeEach(func() {
				sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
				receiver = s.EvmosChain.SenderAccount.GetAddress().String()
				senderAcc = sdk.MustAccAddressFromBech32(sender)
		...
		Describe("to the sender's own eth_secp256k1 account on Qom (sender == recipient)", func() {
			BeforeEach(func() {
				sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.EvmosChain.App.(*app.Qom).RecoveryKeeper.SetParams(s.EvmosChain.GetContext(), params)
				})
			...
			Context("with a sender's claims record", func() {
				Context("without completed actions", func() {
					BeforeEach(func() {
						...
						s.EvmosChain.App.(*app.Qom).ClaimsKeeper.SetClaimsRecord(s.EvmosChain.GetContext(), senderAcc, claim)

						// update the escrowed account balance to maintain the invariant
						err := testutil.FundModuleAccount(s.EvmosChain.GetContext(), s.EvmosChain.App.(*app.Qom).BankKeeper, claimstypes.ModuleName, coins)
				...
				Context("with completed actions", func() {
					...
					BeforeEach(func() {
						amt := sdk.NewInt(int64(100))
						coins := sdk.NewCoins(sdk.NewCoin("aqom", sdk.NewInt(int64(75))))
						claim = claimstypes.NewClaimsRecord(amt)
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						balanceEscrow := s.EvmosChain.App.(*app.Qom).BankKeeper.GetBalance(s.EvmosChain.GetContext(), transfertypes.GetEscrowAddress("transfer", "channel-0"), "aqom")
						Expect(balanceEscrow).To(Equal(coinQom))
						ibcOsmo := s.EvmosChain.App.(*app.Qom).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
						Expect(ibcOsmo.IsZero()).To(BeTrue())

						// Relay both packets that were sent in the ibc_callback
						err := s.pathOsmosisEvmos.RelayPacket(CreatePacket("10000", "aqom", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						...
						ibcEvmos := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, teststypes.AqomIbcdenom)
						Expect(ibcEvmos).To(Equal(sdk.NewCoin(teststypes.AqomIbcdenom, coinQom.Amount)))
					...
					It("should not claim/migrate/merge claims records", func() {
						...
						claimAfter, _ := s.EvmosChain.App.(*app.Qom).ClaimsKeeper.GetClaimsRecord(s.EvmosChain.GetContext(), senderAcc)
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					...
					It("should not recover tokens that originated from other chains", func() {
						// Send uosmo from Osmosis to Qom
						s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.IBCOsmosisChain, "uosmo", 10, sender, receiver, 1)
						...
						ibcAtom := s.EvmosChain.App.(*app.Qom).BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, teststypes.UatomIbcdenom)
						Expect(ibcAtom).To(Equal(sdk.NewCoin(teststypes.UatomIbcdenom, coinAtom.Amount)))

						// Repeat transaction from Osmosis to Qom
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						...
						osmoIBCAtom := s.EvmosChain.App.(*app.Qom).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, teststypes.UatomOsmoIbcdenom)
